/* ===========================================================================
   cursor.js — site-wide custom cursor (shared across all pages)

   A small accent dot tracks the pointer 1:1, trailed by a soft, organic "blob"
   ring (canvas-drawn) that lags behind with eased following. Over any clickable
   link/button the ring swells and wobbles harder while the dot fades out; small
   targets additionally magnetise (the ring snaps to their centre). Ported in
   spirit from umanodesign.studio's CustomCursor.

   Design-system fit: the page is strict ink-on-paper, and its ONE sanctioned
   color is the cursor (DESIGN.md "Single-Ink Rule" exception). The dot/ring use
   the --lens-ember token (#ff6a3d) so the accent is the design's own, never a
   new hue. Decorative only: pointer-events:none, aria-hidden, never intercepts
   input. Gated to fine-pointer + hover devices, and skipped entirely under
   prefers-reduced-motion (a shape chasing the pointer is a vestibular trigger).

   Load once per page near the end of <body>:  <script src="cursor.js" defer></script>
   Pages without tokens.css fall back to the literal accent/ink colours below.
   =========================================================================== */
(() => {
  ...
  // --- Colors pulled from tokens (never hard-coded hues) ----------------
  ...
  const R_STICK  = 28;     // ring radius while expanded over a link (was 36)
  const EASE     = 0.18;   // ring follow speed, free
  const EASE_STK = 0.10;   // ring follow speed, magnetised (snappier to centre)
  ...
  let frame = 0;
  // hot   = over any clickable link → ring grows, wobbles, fills, dot hides
  // stuck = hot AND the target is small → ring also locks onto its centre

  const lerp = (a, b, t) => a + (b - a) * t;

  // 8 wobbling points around a circle → an organic, breathing outline
  const blob = (cx, cy, r, f, hot) => {
    const pts = [];
    for (let i = 0; i < 8; i++) {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    // colour flips to ink over any opted-in dark region (none today, future-proof)
    let dark = false;
    ...
    // (e.g. project cards) get the expand effect but keep following the pointer,
    // because yanking the ring to a big card's centre would jump it far away.
